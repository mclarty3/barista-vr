...
namespace UnitySimpleLiquid
{
    ...
    {
        public MeshRenderer liquidRender = null;
        ...
        [SerializeField]
        [Tooltip("Is liquid a source? (will disable liquid rendering)")]
        private bool _liquidSource;
        ...
        [Tooltip("Places a trigger collider on the surface of the liquid")]
        public bool detectTriggerCollision = false;
        ...
        private void Start()
        {
            ...
            if (detectTriggerCollision)
            {
                if (!liquidRender.TryGetComponent(out MeshCollider _))
                {
                    var trigger = liquidRender.gameObject.AddComponent<MeshCollider>();
                    trigger.convex = true;
        ...
        public Mesh LiquidMesh
        {
            get
            {
                if (meshFilter == null && liquidRender != null)
        ...
        {
            if (liquidRender == null || !liquidRender.enabled)
                return;
        ...
        private void Update()
        {
            if (liquidRender == null || LiquidSource)
                return;
